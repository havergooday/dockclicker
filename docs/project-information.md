...
- `ShipCanvas` 안에서 좌측 격납고 / 중앙 브릿지·파일럿 라운지 / 우측 관제실 3구역을 먼저 배치했다.
- 관제실의 `항성지도` 팝업은 1차 구현 완료 상태다.
- 직접 파견 + 자동 파견 기본 플레이 루프 완성.
- 파츠 구매 → 조립 → 파견 → 귀환 → 수령 → 재투자 전체 사이클 동작.
- 저장/불러오기, 오프라인 진행 계산 구현 완료.
- 다음 구조 목표: 격납고 좌측 확장, 브릿지/파일럿 라운지 연출 강화, 관제실 상점/고용 팝업 이식.
...
- 현재 작업: UI/UX 통합 재설계. 플립 패널 구조를 파노라마 함선 캔버스 + 카메라 이동 구조로 전환하는 단계이며, `ShipCanvas`와 관제실 `항성지도` 팝업 1차 구현이 반영되었다. 기본 선택 위치는 마지막으로 고른 행성으로 복원한다.
