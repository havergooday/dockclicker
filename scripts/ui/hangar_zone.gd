...
func _build() -> void:
	...
	content.add_child(_detail_zone)
...
func _build_background(root: Control) -> void:
	...
	bg_style.bg_color     = Color(0.04, 0.06, 0.10, 0.88)
	bg_style.border_color = Color(0.24, 0.36, 0.54, 0.75)
...
func _detail_locked(vb: VBoxContainer, slot: DispatchManager.AutoSlot) -> void:
	...
	hint.modulate = Color(0.50, 0.52, 0.62)
	vb.add_child(hint)

	var cr := Label.new()
	cr.text = "%s CR" % _fmt(slot.unlock_cost)
	cr.add_theme_font_size_override("font_size", 16)
	cr.modulate = Color(0.85, 0.75, 0.50)
	vb.add_child(cr)
...
func _detail_offline(vb: VBoxContainer, slot: DispatchManager.AutoSlot) -> void:
	...
	var spec := Label.new()
	spec.text = "몸체T%d · 무기T%d · 다리T%d" % [b, w, l]
	spec.add_theme_font_size_override("font_size", 10)
	spec.modulate = Color(0.65, 0.70, 0.82)
	vb.add_child(spec)
	...
	if not idle.is_empty():
		...
		var no := Label.new()
		no.text = "대기 파일럿 없음"
		no.add_theme_font_size_override("font_size", 10)
		no.modulate = Color(0.55, 0.35, 0.35)
		vb.add_child(no)
	...
	dispatch_btn.pressed.connect(func(): navigate_to_control_requested.emit())
...
func _detail_active(vb: VBoxContainer, slot: DispatchManager.AutoSlot) -> void:
	...
	var cr := Label.new()
	cr.text = "예상 수익 %d CR" % preview["credits"]
	cr.add_theme_font_size_override("font_size", 10)
	cr.modulate = Color(0.50, 0.90, 0.55)
	vb.add_child(cr)


func _detail_returned(vb: VBoxContainer, slot: DispatchManager.AutoSlot) -> void:
	var cr := Label.new()
	cr.text = "+ %s CR" % _fmt(slot.credits_earned)
	cr.add_theme_font_size_override("font_size", 18)
	cr.modulate = Color(0.28, 1.00, 0.48)
	vb.add_child(cr)
...
func _make_bay_card(index: int) -> Button:
	...
	if state == "returned":
		...
		var cost := Label.new()
		cost.text = "%s CR" % _fmt(slot.unlock_cost)
		cost.add_theme_font_size_override("font_size", 10)
		cost.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
		cost.modulate = Color(0.50, 0.52, 0.62)
